/**
 * Minimal round-trip test for the Zenith-Link C library.
 * Build: gcc -O2 -Wall -Wextra -o test_zenith_link test_zenith_link.c zenith_link.c -lm
 * Run:   ./test_zenith_link
 */
...
static const uint8_t TEST_KEY[] = "zenith-link-dev-key-2024";
...
static void test_hmac(void)
{
    ...
    uint8_t tag[32];
    zl_hmac_sha256(key, 20, msg, 8, tag);
...
static void test_round_trip(void)
{
    ...
    uint8_t buf[ZL_MAX_FRAME_SIZE];
    int len = zl_encode(buf, sizeof(buf), &state, NULL, 42000,
    ...
    zl_state_t mirror = {0};
    zl_frame_t frame  = {0};
    zl_err_t err = zl_decode(buf, (size_t)len, &frame, &mirror, TEST_KEY, KEY_LEN);
    assert(err == ZL_OK      && "decode failed");
    assert(frame.hmac_ok     && "HMAC verification failed");
    assert(frame.presence == ZL_PRESENCE_ALL && "presence mismatch");
    ...
    /* Tamper with HMAC — should return ZL_ERR_HMAC_FAIL */
    buf[len - 1] ^= 0xFF;
    err = zl_decode(buf, (size_t)len, &frame, &mirror, TEST_KEY, KEY_LEN);
    assert(err == ZL_ERR_HMAC_FAIL && "expected HMAC failure");
    printf("  [PASS] Tampered frame correctly rejected (ZL_ERR_HMAC_FAIL)\n");
}

static void test_delta(void)
{
    zl_state_t prev = {.pitch=10.0f, .rssi=-90.0f, .battery_voltage=24.0f};
    zl_state_t curr = {.pitch=10.0f, .rssi=-88.0f, .battery_voltage=24.1f};

    uint8_t pres = zl_compute_presence(&curr, &prev, false);
    /* pitch unchanged (< 1.5%), rssi changed (2.2%), battery changed (0.4% — below threshold) */
    assert(!(pres & ZL_PRESENCE_ATTITUDE)        && "pitch should NOT be present");
    assert(  (pres & ZL_PRESENCE_RSSI)           && "rssi SHOULD be present");
    printf("  [PASS] Delta presence=0x%02X (rssi changed, pitch stable)\n", pres);
}

int main(void)
{
    printf("Zenith-Link C library tests\n");
    printf("─────────────────────────────────────────────\n");
    test_hmac();
    test_round_trip();
    test_delta();
