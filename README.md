...
│  · Orbitron frames      (Nairobi)     · Nairobi GS     │
...
- **SC-1 spacecraft** — telemetry generation (attitude, power, thermal), Orbitron encoding, HMAC-SHA256 auth, CCSDS Space Packet + TM Transfer Frame wrapping, TC uplink command processing, onboard edge AI health detector, in-orbit compute jobs
...
Orbitron is a presence-bitmask binary protocol. A full telemetry frame is ~78 bytes; a delta (position + power only) is ~60 bytes. The JSON equivalent is ~281 bytes — 72% larger.
...
```bash
go test ./pkg/orbitron -bench=. -benchmem -run TestFrameSize
```
...
```bash
export ORBITRON_HMAC_KEY=your-secret-key-here
...
ORBITRON_HMAC_KEY=dev-key go run ./cmd/spacecraft

# Terminal 2 — ground station
ORBITRON_HMAC_KEY=dev-key SC_ADDR=http://localhost:8080 go run ./cmd/groundstation
...
```bash
while true; do
  curl -s http://localhost:8080/frame/orbitron -o /tmp/f.bin
...
|----------------------|-----------------------|-----------------------------------------|
| `ORBITRON_HMAC_KEY`    | required              | Shared HMAC key                         |
...
| Variable             | Default               | Description                             |
|----------------------|-----------------------|-----------------------------------------|
| `ORBITRON_HMAC_KEY`    | required              | Shared HMAC key — must match spacecraft |
...
| GET    | `/frame/orbitron` |                                           | Raw Orbitron frame          |
...
| POST   | `/receive` | Raw Orbitron frame (`octet-stream`); relay sets `X-Relayed-By` header | Decoded telemetry      |
