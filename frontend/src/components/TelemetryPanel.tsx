...
function Row({ label, value, unit, color = 'var(--text-body)' }: { label: string; value: string; unit: string; color?: string }) {
  return (
    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', padding: '3px 0', borderBottom: '1px solid var(--bg-dark)' }}>
      <span style={{ color: 'var(--text-dim)', fontSize: 9, letterSpacing: 1 }}>{label}</span>
      <span style={{ color, fontSize: 12, fontWeight: 700 }}>{value}<span style={{ color: 'var(--text-dim)', fontSize: 9, marginLeft: 3 }}>{unit}</span></span>
    </div>
  )
}
...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div style={{ marginBottom: 12 }}>
      <div style={{
        color: 'var(--cyan-dim)', fontSize: 9, letterSpacing: 3, marginBottom: 6,
        paddingBottom: 4, borderBottom: '1px solid var(--border)',
        borderLeft: '2px solid var(--cyan-dim)', paddingLeft: 6,
      }}>
        {title}
      </div>
      {children}
...
export function TelemetryPanel({ state }: Props) {
  ...
  const battColor = state && state.battery_voltage > 7.8 ? 'var(--green)' : state && state.battery_voltage > 7.0 ? 'var(--amber)' : 'var(--red)'
  const rssiColor = state && state.rssi > -90 ? 'var(--green)' : state && state.rssi > -110 ? 'var(--amber)' : 'var(--red)'
